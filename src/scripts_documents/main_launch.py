...
for p_inter in [0.3, 0.9]:
    ...
    steps_0_1 = graph_dw.apply_dw(v_convergence=V_CONVERGENCE, confidence_threshold=0.3)
    print(f"Number of steps to convergence: {steps_0_1}")

    print("Applying DW with confidence threshold 0.3...")
    steps_0_3 = graph_dw_clone.apply_dw(v_convergence=V_CONVERGENCE, confidence_threshold=0.5)
    ...
    graph_dw_clone_0_1.apply_dw(v_convergence=V_CONVERGENCE, confidence_threshold=0.3, max_steps=max_steps)
    graph_dw_clone_0_1.draw_opinions(path=f"opinions_history_p_inter_{p_inter}_threshold_0_1.png",
                                     title=f"Simulation of DW with low ε = 0.1 and Inter. prob.={p_inter}")
    graph_dw_clone_0_3.apply_dw(v_convergence=V_CONVERGENCE, confidence_threshold=0.5, max_steps=max_steps)
    graph_dw_clone_0_3.draw_opinions(path=f"opinions_history_p_inter_{p_inter}_threshold_0_3.png",
